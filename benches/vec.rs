...
fn benchmark_pure(c: &mut Criterion) {
    ...
    c.bench_function("pure double", |b| b.iter(|| (black_box(x.clone()), black_box(y.clone()))));
}
...
fn benchmark_zip(c: &mut Criterion) {
    ...
    c.bench_function("zip", |b| b.iter(|| {
        black_box(x.clone().zip_with(y.clone(), fib2))
    }));
    c.bench_function("zip macro", |b| b.iter(|| {
        black_box(vec_utils::zip_with! {
            (x.clone(), y.clone()), |x, y| fib2(x, y)
        });
    }));
    c.bench_function("zip iter", |b| b.iter(|| {
        x.clone()
            .into_iter()
            .zip(y.clone())
            .map(|(x, y)| fib2(x, y))
...
fn benchmark_map(c: &mut Criterion) {
    ...
    c.bench_function("map", |b| b.iter(|| {
        black_box(x.clone().map(fib))
    }));
    c.bench_function("map macro", |b| b.iter(|| {
        black_box(vec_utils::zip_with! {
            x.clone(), |x| fib(x)
        });
    }));
    c.bench_function("map iter", |b| b.iter(|| {
        x.clone()
            .into_iter()
            .map(fib)
            .collect::<Vec<_>>()
